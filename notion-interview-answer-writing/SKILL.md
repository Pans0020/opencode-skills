...
- For embedded/system/concurrency questions, explicitly consider both **Linux** and **MCU / FreeRTOS** contexts when either context could plausibly apply; do not omit one without saying why it does not apply to this exact prompt.
- The **MCU** context must be phrased through **MCU / FreeRTOS** mechanisms when the topic involves tasks, scheduling, interrupts, locks, queues, or real-time behavior.
- Technical symbols, APIs, registers, file names, section names, and OS terms must be formatted as inline code, for example `mutex`, `spinlock`, `FreeRTOS`, `xSemaphoreTake`, `pthread_mutex_lock`, `rt_mutex`, `SCB->VTOR`, `.data`.
- Key concepts that drive the answer must be **bold**, for example **优先级反转**, **优先级继承**, **抢占式调度**, **阻塞态**, **等待队列**, **Linux**, **MCU / FreeRTOS**.
...
- If a plausible **Linux** and **MCU / FreeRTOS** split exists but the answer only covers one side without justification, the answer is invalid.
- If technical symbols are plain text instead of inline code, the answer is invalid.
- If core concepts are not **bolded**, the answer is invalid.
...
Before writing any answer into Notion, verify this checklist for the current single question:

- Did I read the full prompt and any options/context for this exact question?
- Did I identify whether the answer needs **Linux**, **MCU / FreeRTOS**, or both, based on the full prompt rather than one keyword?
- If I omit **Linux** or **MCU / FreeRTOS**, did I state why that context is not applicable to this prompt?
- Did I format technical terms as inline code?
- Did I bold the core concepts that determine the answer?
- Did I include question number, final answer, independent reason, and option-exclusion reasoning?
- Does every reason point back to concrete wording in this question?

If any item is “no”, do not write to Notion yet.
...
4. 只要题目涉及嵌入式系统、调度、锁、中断、任务、实时性、驱动或并发，必须主动检查是否需要同时回答 **Linux** 与 **MCU / FreeRTOS**；不能因为大类是 C/C++ 就跳过系统语境。
5. `dual` 题必须写出两个小标题：`Linux / U-Boot / Kernel 语境` 和 `MCU / FreeRTOS / STM32 语境`。
6. 每个语境都要写成“结论 + 机制 + 怎么做 + 怎么验证 + 常见坑”，不能只列关键词。
7. `Linux` 语境优先讲内核调度、`rt_mutex`、`pthread_mutex`/futex、IRQ 分发、驱动框架、验证手段。
8. `MCU / FreeRTOS` 语境优先讲任务优先级、`FreeRTOS` 调度、`xSemaphoreTake`/mutex、优先级继承、中断约束、队列/信号量、实时性验证。
9. 启动类 MCU 题才讲向量表、链接脚本、Flash/RAM 布局、Bootloader 跳转、`VTOR`、`MSP`、`Reset_Handler`；不要把所有 MCU 题都写成启动链路。
10. `补充 / 追问` 必须围绕本题具体题干产生，不得照搬固定追问。
11. 每个语境小标题、每个回答段落的核心关键词要 **加粗**，例如 **优先级反转**、**优先级继承**、**等待队列**、**启动地址**、**链接脚本**、**中断向量表**、**Bootloader**、**DTB**、**VTOR**。
...
```markdown
题号：<题号>
最终答案：<答案；信息不足时写 信息不足/无法确定；核心概念必须 **加粗**；技术符号必须 `code`>
Linux 语境：<适用时写；不适用时说明为什么本题不需要 Linux 语境>
MCU / FreeRTOS 语境：<适用时写；不适用时说明为什么本题不需要 MCU / FreeRTOS 语境>
...
## Formatting Rules
- 语境标签必须保留原样，但建议用 **加粗** 强调：**Linux / U-Boot / Kernel 语境**、**MCU / FreeRTOS / STM32 语境**。
- 每段第一句最好给出 **一句话结论**，结论里的核心概念要加粗。
- 技术符号、寄存器、段名、文件名必须用 inline code，不要用粗体代替 code。
- 概念词和动词优先加粗，符号优先 code；例如 **优先级反转**、**优先级继承**、**向量表**、**启动地址**、`SCB->VTOR`、`.isr_vector`、`FreeRTOS`、`rt_mutex`、`xSemaphoreTake`。
...
- 对并发/调度/锁题，优先检查是否需要 **Linux** 的 `rt_mutex` / futex / scheduler 和 **MCU / FreeRTOS** 的 mutex / priority inheritance 两套语境。
...
- 对嵌入式并发题只写通用答案，没有区分 **Linux** 和 **MCU / FreeRTOS**。
- 在 MCU 调度题里没有提 `FreeRTOS`、任务优先级、mutex 或优先级继承。
- 技术符号没用 inline code，核心概念没 **加粗**。
