...
- When the authenticated Notion MCP is then used to edit an existing note/page, respect the user's target literally. If they ask to format/fix the existing Notion page, update that exact page in place with `notion_update_page`; do not create a database, child page, or replacement artifact unless explicitly requested.

- Notion-flavored Markdown is not full Markdown. Do not write unsupported heading depths (`#####`/`######`). If fetched page text contains escaped literal Markdown such as `\|`, `\>`, or escaped backticks, unescape it before writing back so MCP can render native tables/blockquotes/code blocks.
